...
<?php if (isset($_POST['btn'])) {
    ...
    $Phone = $_POST['Phone'];
    $Pass = $_POST['Pass']; //123
    $CPass = $_POST['ConfPass']; //123
    $Password = password_hash($Pass, PASSWORD_BCRYPT);
    $CPassword = password_hash($CPass, PASSWORD_BCRYPT);
    $Emailquery = "select * from register where Email = '$Email'"; //abc@gmail.com = xyz@gmail.com
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        if ($Pass === $CPass) {
            $Insquery = "insert into register (Name,PhoneNum,Password,ConfPass,Email) values ('$Name','$Phone','$Password','$CPassword','$Email')";
