...
<?php if (isset($_POST['btn'])) {
    $Email = $_POST['email'];
    $Pass = $_POST['Pass'];
    $EmailSearch = "select * from register where Email = '$Email'"; //abc@gmail.com
    $query = mysqli_query($con, $EmailSearch);
    $EmailCount = mysqli_num_rows($query);
    //1
    if ($EmailCount) {
        $res = mysqli_fetch_assoc($query);
        $db_Pass = $res['Password']; //16 digit
        $Pass_decode = password_verify($Pass, $db_Pass);
        //123
        if ($Pass_decode) {
            echo "<script>alert('Login Succesfull');window.location.href = 'Main.php';</script>";
        } else {
